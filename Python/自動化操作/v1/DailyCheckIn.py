...
def add(page):
    ...
    Settings.add_argument('user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36')
...
class forum:
    ...
    def jkf_use_props(Sc):
        ...
        try:
            WebDriverWait(jkfdriver,30).until(EC.element_to_be_clickable((By.XPATH, "//a[@class='router-link-active router-link-exact-active block lv-2-tab']")))
        except:
            input("等待登入帳號...\n(按下Enter) :")
            os.system("cls")
            jkfdriver.get("https://www.jkforum.net/material/my_item")
...
if __name__ == "__main__":
    ...
    # Jkf論壇使用體力藥水(此腳本就是藥水全部都用完) Sc = 結束後等待幾秒關閉
